...
const App: React.FC = () => {
      ...
      if (savedDraft) {
        ...
        if (parsedData.header && parsedData.sections && parsedData.followUp) {
            if (window.confirm("A saved draft was found. Would you like to restore it?")) {
                ...
                const restoredSections = parsedData.sections.map((section: any) => ({
                    ...section,
  ...
  const saveToLocalStorage = useCallback(debounce(async (data: FormData) => {
    try {
  ...
  const handleFollowUpChange = useCallback((field: string, value: string) => {
    setFormData(prev => ({
      ...prev,
